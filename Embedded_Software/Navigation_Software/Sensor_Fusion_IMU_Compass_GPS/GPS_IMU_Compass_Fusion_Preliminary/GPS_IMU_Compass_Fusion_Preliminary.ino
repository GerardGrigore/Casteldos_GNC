...
// --------------------------------------------------------------------------------------------
// Define the used pins of the Microprocessor and the GPS connection:
...
SoftwareSerial Serial_Connection_GPS(RXPin_GPS, TXPin_GPS);
...
float LATITUDE_REF_BLR = 48.786755;
float LONGITUDE_REF_BLR = 2.321439;
float ALTITUDE_REF_BLR = 58;
...
void setup() {
  ...
  Serial.begin(115200);
  Serial_Connection_GPS.begin(GPSBaud);
...
void loop() {
  ...
  // and altitude, declare:
	while (Serial_Connection_GPS.available() > 0) {
		if (gps.encode(Serial_Connection_GPS.read())) {
      ...
      if (gps.location.isValid()) {
        ...
        // Determine the course provided by the GPS:
        float COURSE_CURRENT;
        COURSE_CURRENT = gps.course.deg();
        // Then print the course:
        Serial.print(F("Course: "));
        Serial.print(COURSE_CURRENT, 6);
        Serial.print(F(","));
