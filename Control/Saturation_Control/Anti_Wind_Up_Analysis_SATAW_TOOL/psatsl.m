function [resu,sysP,sysC,sysD] = psatsl(sysP,sysACT,sysC,sysD,optsat)
%PSATSL  State-feedback design 
%        for a position saturated closed-loop system - LOCAL case
%        Strategy through Sector nonlinearities
%
%[resu,sysP,sysC,sysD] = psatsl(sysP,sysACT,sysC,sysD,optsat) 
%
%   returns 
%   - a state-feedback control gain sysC.Dyu and 
%   - an ellipsoidal set E(inv(W),eta) = {x / x'inv(W) x <= 1/eta). This 
...
%   ---------------------------------------------------------
%   SYSTEM definition (sysP, sysACT, sysC (optional), sysD (optional))
%    
...
%                 1 : optimization of delta (recommanded to set R=I) in
%                     both disturbance cases
%                 2 : optimization of L2-gain gamma from w(t) to z(t). In
%                     case of energy-bounded disturbance only. R must be
%                     equal to I to perserve the L2-significance of the
%                     problem.
...
if isempty(sysC)  
    ...
    if sum(strcmp('Dyw',fieldnames(sysC)))>0
        if size(sysC.Dyw,1) ~= m,   error('sysC.Dyw matrix must have as many rows as columns in sysP.Bu.');end
        if size(sysC.Dyw,2) ~= q,   error('sysC.Dyw matrix must have as many columns as sysP.Bw.');end
    else
        sysC.Dyw = zeros(m,q);
    end
end
...
q = size(sysP.Bw,2);  % For the case where q is forced to 0... although not used in the sequel
...
% Lyapunov condition    
if ~isequal(optsat.D,2)
    Mlyap = [Ap*W+W*Ap'+Bp*Y+Y'*Bp'+tau1*W   Bp*S-Y'-Z'    Bwbar;...
             S*Bp'-Y-Z                         -2*S       -Dcw;...
             Bwbar'                            -Dcw'      -tau2*R];
else
    Mlyap = [Ap*W+W*Ap'+Bp*Y+Y'*Bp'+tau1*W   Bp*S-Y'-Z'    Bwbar        W*Cz'+Y'*Dz';...
             S*Bp'-Y-Z                         -2*S       -Dcw          S*Dz';...
...
% Inclusion condition
for i = 1:m
   MInc = [W Z'*Im(:,i);Im(i,:)*Z eta*u0(i)^2];
   pblmi = [pblmi, MInc >= 0];
end
...
% Display the results
% -------------------
message = 'LOCAL SYNTHESIS (state feedback)';
switch optsat.W, 
