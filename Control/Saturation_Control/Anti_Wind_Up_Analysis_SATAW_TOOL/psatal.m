function [resu,sysP,sysC,sysD] = psatal(sysP,sysACT,sysC,sysD,optsat)
%PSATAL  Stability analysis 
%        of a position saturated closed-loop system - LOCAL case
%        Strategy through Sector nonlinearities
%
%[resu,sysP,sysC,sysD] = psatal(sysP,sysACT,sysC,sysD,optsat) 
%
...
% ------------
sysP = testsysp(sysP);
[n,m] = size(sysP.Bu);  p = size(sysP.Dyw,1);  
% test of sysACT
% --------------
...
% Lyapunov condition    
Mlyap = [Abar*W+W*Abar'+tau1*W   Bphi*S-W*Kbar'-Z'   Bwbar;...
         S*Bphi'-Kbar*W-Z       -2*S-Dphi*S-S*Dphi'  -Dwbar;...
         Bwbar'                 -Dwbar'              -tau2*R];
pblmi = [pblmi, Mlyap <= 0];
