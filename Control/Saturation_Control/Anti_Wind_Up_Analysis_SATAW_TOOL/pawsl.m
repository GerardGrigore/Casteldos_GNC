function [resu,sysP,sysC,sysD,sysAW] = pawsl(sysP,sysACT,sysC,sysD,sysAW,optsat)
%PAWSL   Anti-windup - Static Anti-Windup synthesis
%        Position saturated closed-loop system with AW - LOCAL case
%        Strategy through Sector nonlinearities
%
%[resu,sysP,sysC,sysD,sysAW] = PawSL(sysP,sysACT,sysC,sysD,sysAW,optsat)
%
%   returns: 
%   * the static anti-windup compensator sysAW,
%   * an ellipsoidal set E(inv(W),1) = {x / x'inv(W) x <= 1) 
...
%        optsat.condEc: conditioning of Ec. Specify the limit values.
%                 0 = no conditioning   ----- Default -------                 
...
if nargin > 5  && ~isempty(optsat) % optsat is at least partially defined
    ...
    if isfield(optsat,'Xi0') 
        if optsat.W == 3
            Xi0 = optsat.Xi0;
            if size(Xi0,1) ~= np+nc, error('The columns of optsat.Xi0 should be of length np+nc.'); end
    ...
    if isfield(optsat,'condEc')
        if ~isnumeric(optsat.condEc) || optsat.cond < 0
            fprintf('Warning: optsat.condEc should be a positif value. The Default value (no conditionning) is forced.\n');
            fprintf(' \n');
            optsat.condEc = 0;
        end
    else
...
% ------------------------
S = [];
for ii=1:m,  S = [S;Im(ii,:)*sdpvar(1,1,'symmetric')]; end
...
% Inclusion condition
for ii = 1:m
   MInc = [Q Z'*Im(:,ii);Im(ii,:)*Z delta*u0(ii)^2];
   pblmi = [pblmi, MInc >= 0];
end
...
% Condtionning of Ec
Inc = eye(nc);
if optsat.condEc > 0 && ~isequal(optsat.AW,2)
   for jj = 1:m
        for ii = 1:nc
            MEc = [Im(jj,:)*S*Im(:,jj)*optsat.condEc Inc(ii,:)*ZD(1:nc,:)*Im(:,jj);...
                   Im(jj,:)*ZD(1:nc,:)'*Inc(:,ii)       Im(jj,:)*S*Im(:,jj)]; 
%             MEc = [Im(j,:)*S*Im(:,j)*optsat.condEc Inc(i,:)*[eye(nc) zeros(nc,m)]*ZD*Im(:,j);...
%                    Im(j,:)*ZD'*[eye(nc) zeros(nc,m)]'*Inc(:,i)       Im(j,:)*S*Im(:,j)]; 
            pblmi = [pblmi, MEc >= 0];
        end
   end    
end

% Conditions related to the optimization on Q
switch optsat.W
    case 2,    McondOpt = [mu*eye(np+nc+na) eye(np+nc+na);...
                           eye(np+nc+na)    Q];
               pblmi = [pblmi, McondOpt >= 0];                  
    case 3,    lmicondOpt = set('');
               for ii=1:size(optsat.Xi0,2)
                   McondOpt = [mu optsat.Xi0(:,ii)';optsat.Xi0(:,ii) Q];
                   pblmi = [pblmi, McondOpt >= 0];
               end 
end
...
% Display the results
% -------------------
message = 'LOCAL SYNTHESIS of a STATIC anti-windup compensator';
switch optsat.W, 
...
switch sysD.opt
   ...
   case 'energy', message2 = 'Energy bounded disturbance';
    case 'amplitude', message2 = 'Amplitude bounded disturbance not managed by the function';
end
